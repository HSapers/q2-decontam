...
plugin.methods.register_function(
    ...
    parameters={'sample_metadata': Metadata}, #why is this a parameter and not an input? primative type?
    input_descriptions={'table': 'the feature table to be filtered'},
    parameter_descriptions={'sample_metadata': 'metadata'}, #k is what it is in the function, v is the type
    outputs=[('ids_to_filter', Filter)],
    name='remove singletons',
    description='removes ASVs with 0 reads or singleton ASVs that may have been introduced during up'
